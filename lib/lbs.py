...
def global_rigid_transformation(pose, J, kintree_table, xp):
    ...
    if xp == chumpy:
        ...
        def rodrigues(x): return Rodrigues(x)
    else:
        import cv2
        def rodrigues(x): return cv2.Rodrigues(x)[0]

    def with_zeros(x): return xp.vstack((x, xp.array([[0.0, 0.0, 0.0, 1.0]])))
    ...
    def pack(x): return xp.hstack([np.zeros((4, 3)), x.reshape((4, 1))])
