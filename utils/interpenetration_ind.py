...
def get_nearest_points_and_normals(vert, base_verts, base_faces):
    ...
    cl_edg_idxs = np.nonzero((nearest_part <= 3) & (nearest_part > 0))[
        0].astype(np.int)
...
def remove_interpenetration_fast(mesh, base, L=None):
    ...
    direction = np.sign(np.sum((mesh.v - nearest_points)
                               * nearest_normals, axis=-1))
...
if __name__ == '__main__':
    ...
    mvs = MeshViewers((1, 2))
    mesh1.set_vertex_colors_from_weights(
        np.linalg.norm(mesh.v - mesh1.v, axis=1))
    mesh.set_vertex_colors_from_weights(
        np.linalg.norm(mesh.v - mesh1.v, axis=1))
